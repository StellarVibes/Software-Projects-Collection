# Boost 搜索引擎 - Linux 版本文档

## 项目概述

Boost 搜索引擎是一个基于 C++ 开发的轻量级搜索引擎，使用倒排索引技术实现高效的文本搜索功能。项目采用模块化设计，包含 HTML 解析、索引构建、搜索服务和 HTTP 服务器等核心模块。本文档专门针对 Linux 平台提供详细的安装、编译和使用说明。
...
# 1. 安装依赖
sudo apt-get update
sudo apt-get install -y cmake g++ libboost-all-dev libjsoncpp-dev
...
**输出示例：**
```
[NORMAL][1640987654]获取index单例成功....
[NORMAL][1640987654]建立正排和倒排索引成功....
[NORMAL][1640987654]当前的已经建立的索引文档 : 50
[NORMAL][1640987654]当前的已经建立的索引文档 : 100
[NORMAL][1640987654]当前的已经建立的索引文档 : 150
...
[NORMAL][1640987654]当前的已经建立的索引文档 : 8550
```
...
如果需要修改端口，编辑 `http_server.cpp`：

```cpp
// 修改第 42 行
svr.listen("0.0.0.0", 8080);  // 改为其他端口，如 8081
```
...
ExecStart=/path/to/Boost_Searcher/build/http_server
Restart=on-failure
RestartSec=10
...
# 启用编译器优化
cmake -B build -DCMAKE_CXX_FLAGS="-O3 -march=native"

# 使用 LTO 链接时优化
cmake -B build -DCMAKE_INTERPROCEDURAL_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS="-flto"
```
...
```bash
# 设置进程优先级
nice -n -10 ./build/http_server

# 限制 CPU 使用
taskset -c 0,1 ./build/http_server

# 使用大页内存
export HUGETLB_MOREMEM=yes
./build/http_server
```
...
```bash
# 重定向到文件
./build/http_server > server.log 2>&1

# 使用 tee 同时输出到屏幕和文件
./build/http_server 2>&1 | tee server.log
```
...
Boost 搜索引擎是一个功能完整、设计合理的轻量级搜索引擎项目。通过模块化的架构设计，实现了从 HTML 解析到搜索服务的完整流程。项目采用 C++ 标准库和 Boost 库，保证了代码的高性能和可维护性。本文档针对 Linux 平台提供了详细的安装、编译和使用说明，帮助用户快速上手和部署项目。
...
**文档版本**：1.0  
**最后更新**：2024-01-24  
**适用平台**：Linux (Ubuntu/Debian/CentOS/RHEL/Fedora/Arch)
